      ...
      <div className="content flex flex-col flex-[1_1_auto] items-center justify-center m-0">
        <div className="flex-col md:flex-row items-center text-center mb-[clamp(2rem,5vw,4rem)] p-[clamp(1rem,3vw,2rem)]">
          ...
          <div className="w-full text-center md:text-left">
            ...
            <h5 className="text-gray-500 text-lg mb-3">
              {aboutData.title}
            </h5>
          </div>
        </div>
        <div className="w-[90%] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 rounded-[clamp(6px,1.5vw,10px)] ">
          {data.map((item, index) => (
            <a
              key={index}
              href={item.href}
              className="relative aspect-square rounded-xl overflow-hidden shadow-md group transform transition duration-300 hover:scale-105 hover:shadow-xl cursor-pointer mx-2"
            >
              <img
